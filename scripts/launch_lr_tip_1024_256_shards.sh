...
for SHARD in 0 1 2 3 4 5 6 7; do
  ...
  tmux new -d -s "${SESSION}" \
    "cd /data_b/qtwei/ckcyi/AGOPD && CUDA_VISIBLE_DEVICES=${GPU} .venv/bin/python -m agopd.experiments.offline_lr_tip_eval --prompts-file ${PROMPTS_FILE} --limit 1024 --num-shards 8 --shard-index ${SHARD} --max-new-tokens 256 --execution-mode joint --device cuda --dtype float16 --budgets 0.05,0.1,0.2 --output-kl-direction forward --progress-every 8 --output-dir ${OUT_DIR} > ${OUT_DIR}/run.log 2>&1"
